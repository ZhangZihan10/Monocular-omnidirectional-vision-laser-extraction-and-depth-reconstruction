...
red_rgb = [80/255, 5/255, 30/255]; % RGB值转换到0-1范围
...
lines = houghlines(cleanedImage, theta, rho, peaks, 'FillGap', 10, 'MinLength', 25);
...
imshow(black_image);
hold on;
imageHeight = 1080;  % 假定图像高度为1080像素
Points = zeros(length(lines), 2);  % 初始化存储所有中心点的数组
for k = 1:length(lines)
    xy = [lines(k).point1; lines(k).point2];
    % 计算中心点
    centerX = mean(xy(:,1));
    centerY = mean(xy(:,2));  % 反转y坐标
    Points(k, :) = [centerX, imageHeight - mean(xy(:,2))];  % 存储每个中心点的坐标
    plot(centerX, centerY, 'o', 'LineWidth', 2, 'Color', 'blue');
end
hold off;
title('Center Line of Detected Lines');

% 使用DBSCAN算法进行聚类，Epsilon设置为30
epsilon = 30;  % 点与点之间的最大距离
minpts = 2;  % 形成一个簇所需的最小点数
[labels2, corepts] = dbscan(Points, epsilon, minpts);

% 显示聚类结果
figure;
gscatter(Points(:,1), Points(:,2), labels2);
title('Clustered Points');

% 计算每个类别中的平均坐标
uniqueLabels = unique(labels2);
averagePoints = zeros(length(uniqueLabels), 2);
for i = 1:length(uniqueLabels)
    label = uniqueLabels(i);
    indices = labels2 == label;
    averagePoints(i, :) = mean(Points(indices, :), 1);
end
...
imshow(black_image);
hold on;
for i = 1:size(averagePoints, 1)
    %plot(1050, imageHeight -30.5, 'o', 'LineWidth', 2, 'Color', 'blue');
    plot(averagePoints(i, 1), imageHeight -averagePoints(i, 2), 'o', 'LineWidth', 2, 'Color', 'blue');
end
hold off;
title('Average Points of Each Cluster');
