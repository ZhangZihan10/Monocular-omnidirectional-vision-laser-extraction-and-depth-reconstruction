...
% 显示处理后的图像
figure;
imshow(cleanedImage4);
title('Other Laser Image');%title('Medium light Image');
...
% 显示处理后的图像
figure;
imshow(cleanedImage5);
title('Yellow reflective area');%title('Medium light Image');
...
% 叠加二值图像  combinedImage =cleanedImage;
...
i=[700;1200]; % working image region - column
j=[50;500]; % working image region - row


Cube_l=10;%目标物体边长一半，单位mm
[C_Up] = cube_dist(binaryImage,i,j,x,y,las_dist,ocam_model);
[potNew,xt1,yt1]=MappingRobotR(C_Up,Cube_l);
%C_Up1 = mean(C_Up(:,1));%调用x的信息
%C_Up2 = mean(C_Up(:,2));%调用y的信息
%potNew=PositionTranR(C_Up1,C_Up2);%450为虚拟方形的边长一半

[C_Up] = cube_dist(img,i,j,x,y,las_dist,ocam_model);
[potOld,xt2,yt2]=MappingRobotR(C_Up,Cube_l);
%C_Up1 = min(C_Up(:,1));%调用x的信息
%C_Up2 = min(C_Up(:,2));%调用y的信息
%C_Up1 = mean(C_Up(:,1));%调用x的信息
%C_Up2 = mean(C_Up(:,2));%调用y的信息
%potOld=PositionTranR(C_Up1,C_Up2);
%绘制新旧方法对比图
figure;
